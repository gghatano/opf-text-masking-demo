...
- 📘 **確定事実**: OPF は **LoRA 非対応＝フルFT のみ**。18ラベルは `--label-space-json` で直接学習可。指標は `detection.span.*`/`by_class.<label>.span.*` を台帳へ写像 [\[2\]](#ref2)。
...
🔎 **結論**: 守備範囲が相補的。**OPF（or 正規表現）× GiNZA のアンサンブル**が18ラベルの広いカバレッジに有効と見込まれる（B1/#10 の設計へ反映）。各手法の生データ・型対応は手法ページへ。
