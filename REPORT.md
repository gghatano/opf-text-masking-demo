...
- 🔎 **GiNZA 比較（定性）**: GiNZA は**年齢・和暦日付・職業**を拾い（OPFの弱点）、電話/メールは OPF/正規表現が強い。**相補的**＝用途で使い分け・B1の正規表現補完の根拠（アンサンブルは今回スコープ外, 図1）。詳細 → [GiNZA ページ](methods/ginza.md)。
...
🔎 **結論**: 守備範囲が相補的。構造化PIIは **OPF＋正規表現**(B1)、準識別子は **GiNZA等の日本語NER** が担う——という**用途の使い分け**が有効（アンサンブル＝統合モデルは今回スコープ外 #18）。各手法の生データ・型対応は手法ページへ。
