...
単一モデルの絶対値だけでは見えない強み・弱みを明らかにするため、**比較対象の評価は本研究の重要な構成要素**である。比較対象として日本語固有表現抽出ライブラリ **GiNZA**（`ja_ginza`）を、OPF と同一の評価データ・一致基準・指標で**実装・評価した**（実装: `scripts/03_compare_models.py`、結果は 3.5 節）。GiNZA は汎用の固有表現抽出器であり、PII 検出専用の OPF とは設計目的が異なる。両者を同条件で比較することで、検出対象ごとの相補性（OPF は構造化 PII に、GiNZA は人名・準識別子に強い、等）を実数で示す。現時点の GiNZA 評価は小規模パイロット（合成 15 文書）であり、評価データ 300 文書での本比較は今後実施する。さらに Presidio・GLiNER・日本語 NER モデルを同一基準で順次追加する。
